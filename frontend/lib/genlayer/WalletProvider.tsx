...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const handleAccountsChanged = async (...args: unknown[]) => {
      ...
      // Empty accounts during chain switch are often transient — re-check provider.
    ...
    const handleChainChanged = async (...args: unknown[]) => {
      ...
      // Brief pause so MetaMask finishes the switch before eth_accounts.
    ...
    const handleDisconnect = async () => {
      // MetaMask often emits "disconnect" on chain switches — don't drop UI session
      // if accounts are still authorized.
